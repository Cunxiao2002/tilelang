...
def grouped_gemm(batch_sizes_list,
    ...
    return persistent_kernel if use_persistent else non_persistent_kernel
...
def run_tilelang_grouped_gemm(batch_sizes_list,
    ...
    batch_sum = sum(batch_sizes_list)
    kernel = grouped_gemm(
        tuple(batch_sizes_list), K, M, block_M, block_N, block_K, num_stages, threads, "float16", use_persistent)
