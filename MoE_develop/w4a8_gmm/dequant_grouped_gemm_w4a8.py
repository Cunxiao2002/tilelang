...
def torch_gmm_w4a8(
    ...
    b_unpacked = torch.stack([torch_convert(b_i) for b_i in b])
...
def grouped_gemm_w4a8(
    batch_sizes_list, K, N, block_M, block_N, block_K, num_stages=2, threads=128, in_dtype="int8", out_dtype="bfloat16", num_bits=4
):
    """
    args:
    ...
    def w4a8_kernel(
        ...
        with T.Kernel(
            ...
            for k in T.Pipelined(T.ceildiv(K, block_K), num_stages=num_stages):
                ...
                # T.print(B_dequant_local)
                T.copy(B_dequant_local, B_dequant_prev_local)
                T.gemm(A_shared, B_dequant_prev_local, C_local, transpose_B=True)
...
def construct_inputs(batch_sizes_list, K, M, trans_b, padding_M, device, dtype):
    ...
    # A = torch.full((batch_sum, K), 1, device=device, dtype=torch.bfloat16)
    ...
    # B = torch.full((batch_count, M, K // 2), 17, device=device, dtype=torch.uint8)
    # B[1][1][1] = 52
...
def run_tilelang_grouped_gemm(
    ...
    torch.testing.assert_close(out, ref_output)
