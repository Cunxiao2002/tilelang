...
def matmul_int8xint4(M, N, K, in_dtype, out_dtype, accum_dtype, block_M, block_N, block_K, num_stages, num_bits, threads):
    # K是解包后的K
    # 因此解包前是K // 2
    num_elems_per_byte = 8 // num_bits
    storage_dtype = "uint8"
...
def assert_matmul_int8xint4_correctness(M, N, K, in_dtype, out_dtype, accum_dtype, block_M, block_N, block_K, num_stages, num_bits, threads):
    kernel = matmul_int8xint4(M, N, K, in_dtype, out_dtype, accum_dtype, block_M, block_N, block_K, num_stages, num_bits, threads)
...
# assert_matmul_int8xint4_correctness(256, 256, 256, "int8", "int32", "int32", 64, 64, 64, 1, 4, 128)
assert_matmul_int8xint4_correctness(8192, 8192, 8192, "int8", "int32", "int32", 64, 64, 64, 1, 4, 128)
